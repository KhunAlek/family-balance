...
test('0016 adds only empty durable relationships and does not backfill or alter existing facts',t=>{
  const {raw}=createSeededSqliteD1();t.after(()=>raw.close());for(const name of migrations.slice(0,-3))raw.exec(fs.readFileSync(new URL(`../migrations/${name}`,import.meta.url),'utf8'));
