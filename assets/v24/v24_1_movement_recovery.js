...
function movementAttempt(movement) {
  ...
  const attempt = { requestId: secureMovementRequestId(), movement };
...
async function submitRecordedMovement(movement, messageId) {
  ...
  try { result = await postAction({ ...movement, requestId: attempt.requestId }); }
    ...
    if (error?.status === 400) try { finishMovementAttempt(attempt.requestId); } catch {}
    setMsg(messageId, movementErrorMessage(error));
    renderMovementRecoveryBanner();
    return false;
  }
...
function renderMovementRecoveryBanner() {
  ...
  if (!records.length && !movementRecoveryNotice) { banner.classList.remove('show'); return; }
  const message = document.createElement('span');
  if (movementRecoveryNotice) message.textContent = movementRecoveryNotice;
  else {
    message.appendChild(document.createTextNode(movementDescription(records[0].movement)));
  ...
  if (records.length && !movementRecoveryBusy) {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'btn secondary';
    button.textContent = 'Check and retry';
    button.addEventListener('click', () => retryPendingMovement(records[0].requestId));
  ...
  try {
    ...
    if (!status.found) {
      ...
      if (!result.ok) {
        ...
        movementRecoveryNotice = result.error || 'The movement was rejected without being recorded.';
        return;
      }
    }
    recorded = true;
    ...
    if (!recorded && error?.status === 400) try { finishMovementAttempt(attempt.requestId); } catch {}
