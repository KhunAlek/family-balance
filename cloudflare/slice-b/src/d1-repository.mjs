export async function loadFinancialSnapshot(db, householdId = 'family') {
  ...
  const statements = [
    ...
    db.prepare('SELECT * FROM obligations WHERE household_id=? ORDER BY name').bind(householdId),
    db.prepare('SELECT * FROM obligation_payments WHERE household_id=? ORDER BY payment_date,payment_id').bind(householdId),
    db.prepare('SELECT name,target_amount_satang,priority_rank,status,target_date,cycle_commitment_satang FROM goals WHERE household_id=? ORDER BY priority_rank,name').bind(householdId),
  ...
  return {
    ...
    ...(rows(12).some(r=>r.name==='fixed_expense_management_enabled') ? { fixedExpenseEnabled: true } : {}),
    config,
    salaryCycle,
