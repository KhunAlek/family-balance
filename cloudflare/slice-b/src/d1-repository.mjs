export async function loadFinancialSnapshot(db, householdId = 'family') {
  ...
  const statements = [
    ...
    db.prepare('SELECT business_date,sheet_order,alex_balance_satang,olga_balance_satang,one_off_payment_name,one_off_payment_amount_satang,one_off_payment_account,income_receipt_source,income_receipt_amount_satang,source_row FROM balance_history WHERE household_id=? ORDER BY business_date,sheet_order').bind(householdId),
    ...
    db.prepare('SELECT source,business_date,amount_satang,lands_in FROM income_receipts WHERE household_id=? ORDER BY business_date,receipt_id').bind(householdId),
    ...
    db.prepare('SELECT business_date,sheet_order,account,direction,amount_satang,source_row FROM ledger_movements WHERE household_id=? ORDER BY business_date,sheet_order').bind(householdId),
